...
interface DepartmentTableDialogProps {
  openDialog: boolean;
  imageURL: string;
  currentYear: number;
  indicatorID: number;
...
const DepartmentTableDialog: FC<DepartmentTableDialogProps> = (props) => {
  const {
    currentYear,
    imageURL,
    indicatorID,
  ...
  return (
    <Dialog fullWidth maxWidth="xl" open={openDialog} onClose={onClose}>
      <Box sx={{ p: 3, backgroundColor: LIGHT.main }}>
        <AvatarTitle
          imageURL={imageURL}
          subHeader={`Data Departemen Teknik Informatika tahun ${currentYear}`}
        ...
        >
          ...
          Departemen. Untuk melihat data indikator pada level Fakultas, silahkan
          klik{' '}
          <Link
...
export default DepartmentTableDialog;
