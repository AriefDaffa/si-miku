...
interface MajorTableDialogProps {
  openDialog: boolean;
  imageURL: string;
  currentYear: number;
  indicatorID: number;
...
const MajorTableDialog: FC<MajorTableDialogProps> = (props) => {
  const {
    currentYear,
    imageURL,
    indicatorID,
    ...
    <Dialog fullWidth maxWidth="xl" open={openDialog} onClose={onClose}>
      <Box sx={{ p: 3, backgroundColor: LIGHT.main }}>
        ...
        >
          ...
          Program Studi. Untuk melihat data indikator pada level Fakultas,
          silahkan klik{' '}
          <Link
...
export default MajorTableDialog;
